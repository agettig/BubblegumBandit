...
import static edu.cornell.gdiac.bubblegumbandit.controllers.ai.graph.TiledGraph.GRAVITY_DOWN_RESTRICTED;
import static edu.cornell.gdiac.bubblegumbandit.controllers.ai.graph.TiledGraph.GRAVITY_UP_RESTRICRED;
 ...
 *
 * @author davebaol */
public class ModifiedIndexedPathFinder<N> implements PathFinder<N> {
    ...
    public ModifiedIndexedPathFinder(IndexedGraph<N> graph) {
        this(graph, false);
    }


    public ModifiedIndexedPathFinder(IndexedGraph<N> graph, boolean calculateMetrics) {
        this.graph = graph;
    ...
    protected void visitChildren (N endNode, Heuristic<N> heuristic) {
        ...
        TiledNode currNode = (TiledNode) current.node;
    ...
    /** A class used by {@link ModifiedIndexedPathFinder} to collect search metrics.
     *
     * @author davebaol */
