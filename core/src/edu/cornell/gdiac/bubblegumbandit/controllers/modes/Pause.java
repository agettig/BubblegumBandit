 ...
 * loading screen.
 */
public class Pause implements Screen, InputProcessor, ControllerListener {
     ...
     */
    public Pause(GameCanvas canvas, int millis) {
        this.canvas = canvas;
        budget = millis;
    ...
    private Color getButtonTint(String buttonName) {
        ...
        if (buttonName.equals("start")) {
            if (hoveringResume && pressState == 1) return pressTint;
            else if (hoveringResume) return hoverTint;
            else return defaultTint;
        }

        if (buttonName.equals("level")) {
            if (hoveringRestart && pressState == 2) return pressTint;
            else if (hoveringRestart) return hoverTint;
            else return defaultTint;
        }

        if (buttonName.equals("settings")) {
            if (hoveringLevelSelect && pressState == 3) return pressTint;
            else if (hoveringLevelSelect) return hoverTint;
            else return defaultTint;
        }

        if (buttonName.equals("exit")) {
            if (hoveringSettings && pressState == 4) return pressTint;
