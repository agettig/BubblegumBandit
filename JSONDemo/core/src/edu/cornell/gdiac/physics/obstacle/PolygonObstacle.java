 ...
 * Modified to support custom debug colors and dynamic reshaping
 * Version: 3/2/2016
 */
...
public class PolygonObstacle extends SimpleObstacle {
	...
	protected void createFixtures() {
		...
		// Create the fixtures
		if (geoms.length != shapes.length) {
			geoms = new Fixture[shapes.length];
		}
		for(int ii = 0; ii < shapes.length; ii++) {
	...
	public void draw(GameCanvas canvas) {
		if (region != null) {
			canvas.draw(region,Color.WHITE,0,0,getX()*drawScale.x,
